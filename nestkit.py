...
def priormap(theta):
    ...
    gases_myP, chemeq, dist, dist_err, cloudtype, do_clouds, gasnum, gaslist, cloudnum, inlinetemps, coarsePress, press, inwavenum, linelist, cia, ciatemps, fwhm, obspec, proftype, do_fudge, prof, do_bff, bff_raw, ceTgrid, metscale, coscale = settings.runargs
    ...
    if (do_fudge == 1):
        ...
        s1 = np.where(obspec[0, :] < 5.2)
        s2 = np.where(obspec[0, :] > 5.2)

        # logf here
        minerr1 = np.log10((0.01 * np.min(obspec[2, s1])) ** 2.)
        maxerr1 = np.log10((100. * np.max(obspec[2, s1])) ** 2.)

        minerr2 = np.log10((0.01 * np.min(obspec[2, s2])) ** 2.)
        maxerr2 = np.log10((100. * np.max(obspec[2, s2])) ** 2.)

        phi[ng + 3] = (theta[ng + 3] * (maxerr1 - minerr1)) + minerr1
        phi[ng + 4] = (theta[ng + 4] * (maxerr2 - minerr2)) + minerr2
...
def modelspec(theta, args, gnostics=0):
    gases_myP, chemeq, dist, dist_err, cloudtype, do_clouds, gasnum, gaslist, cloudnum, inlinetemps, coarsePress, press, inwavenum, linelist, cia, ciatemps, fwhm, obspec, proftype, do_fudge, prof, do_bff, bff_raw, ceTgrid, metscale, coscale = args
    nlayers = press.size
    ...
    cfunc = cf[:npatches, :nwave, :nlayers]
...
def countdims(runargs, plist=False):
    gases_myP, chemeq, dist, dist_err, cloudtype, do_clouds, gasnum, gaslist, cloudnum, inlinetemps, coarsePress, press, inwavenum, linelist, cia, ciatemps, fwhm, obspec, proftype, do_fudge, prof, do_bff, bff_raw, ceTgrid, metscale, coscale = runargs
    ...
    if (do_fudge == 1):
        ...
        pnames.extend(['dlam', 'logb1', 'logb2'])
        pc = ng + 5
    else:
        pnames.append('dlam')
        pc = ng + 3
